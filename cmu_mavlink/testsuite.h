...
static void mavlink_test_pwm_final_input(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_PWM_FINAL_INPUT >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_pwm_final_input_t packet_in = {
        93372036854775807ULL,{ 201.0, 202.0, 203.0, 204.0, 205.0, 206.0, 207.0, 208.0}, 20778
    };
    mavlink_pwm_final_input_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.timestamp = packet_in.timestamp;
        packet1.seq_id = packet_in.seq_id;
        
        mav_array_memcpy(packet1.pwm_final, packet_in.pwm_final, sizeof(float)*8);
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_PWM_FINAL_INPUT_MIN_LEN + (char *)&packet1, 0, 
            sizeof(packet1) - MAVLINK_MSG_ID_PWM_FINAL_INPUT_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_pwm_final_input_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_pwm_final_input_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_pwm_final_input_pack(system_id, component_id, &msg , packet1.timestamp , packet1.seq_id , packet1.pwm_final);
    mavlink_msg_pwm_final_input_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_pwm_final_input_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.timestamp , packet1.seq_id , packet1.pwm_final);
    mavlink_msg_pwm_final_input_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_pwm_final_input_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_pwm_final_input_send(MAVLINK_COMM_1 , packet1.timestamp , packet1.seq_id , packet1.pwm_final);
    mavlink_msg_pwm_final_input_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_cmu_mavlink(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_pwm_final_input(system_id, component_id, last_msg);
}
